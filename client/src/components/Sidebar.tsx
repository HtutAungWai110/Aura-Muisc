...
const DesktopNavItem = ({ icon, label, isActive, path }: NavItemProps) => (
  <Link
    to={path}
...
const MobileNavItem = ({ icon, label, isActive, path }: NavItemProps) => (
  <Link
    to={path}
...
const Sidebar = () => {
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex fixed inset-y-0 left-0 z-50 flex-col h-full w-80 rounded-r-lg bg-surface-container/90 backdrop-blur-2xl shadow-2xl overflow-hidden pb-24">
        {/* Profile Section */}
      ...
      >
        <div className="relative group">
          ...
          <img
            src={avatar}
            alt={displayName}
            className="relative w-10 h-10 rounded-full object-cover border-2 border-surface-container"
          />
        </div>
      </Link>

      {/* Mobile Bottom Navigation */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-[110] bg-surface-container/95 backdrop-blur-2xl border-t border-white/5 px-2 py-2">
